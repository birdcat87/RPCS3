...
public:
	...
	const struct settings
	{
		...
		const QString anti_aliasing              = tr("Emulate PS3 multisampling layout.\nCan fix some otherwise difficult to solve graphics glitches.\nLow to moderate performance hit depending on your GPU hardware.\nDisable if using the Intel iGPU ANV Vulkan driver as the hardware does not support shaderStorageImageMultisample, which is needed for RPCS3 to emulate the Playstation 3's Anti-Aliasing (MSAA).");
